...
char* test_num = (char*)malloc(sizeof(char) * 6);
    ...
    if (pl->stack_size < num_park) {
        float test_start_time_1;
        printf("输入测试用开始计时时间\n");
        if (scanf("%f", &test_start_time_1) != 1) {
            printf("输入测试用开始时间错误\n");
            while (getchar() != '\n');
            return 0;
        }
...
int count_fee(Parking_space* ps) {
    int delta_time = (int)(time_now - ps->test_start_time);
    if (delta_time <= 3) {
        printf("车牌为%s的车收费%d元\n", ps->car.num,5 * delta_time);
    }
    else {
        printf("车牌为%s的车收费%d元\n", ps->car.num, 15 + (delta_time - 3) * 10);
    }
    return 1;
}
...
void PrintParkinglot(Parking_lot *pl){
    printf("当前停有%d辆车，剩余%d车位\n", pl->stack_size, num_park - pl->stack_size);
            ...
            for (int i = 0; i < pl->stack_size; i++) {
                printf("%s   %f\n", temp->car.num,time_now-temp->test_start_time);
                temp = temp->next;
            }
}
//打印过道信息
